        ...
        if row[0] == "sceneInfo":
            ...
            for sent in scene_info:
                ...
                if len(present_chars) == 0:
                    if last_char1 != "":
                        final_data.append((sent_tokens, min(last_char1, last_char2), max(last_char1, last_char2)))
                elif len(present_chars) == 1:
                    if last_char1 != "":
                        final_data.append((sent_tokens, min(present_chars[0], last_char1), max(present_chars[0], last_char1)))
                        last_char2 = last_char1
                        last_char1 = present_chars[0]
                else:
                    final_data.append((sent_tokens, min(present_chars[0], present_chars[1]), max(present_chars[0], present_chars[1])))
                    last_char1 = present_chars[0]
